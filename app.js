...
app.get('/total-value/:address', (req, res) => {
	...
	getAssetsValue(address)
		.then(async (response) => {
			let totalValue = calculateTotalValue(response.data);
			console.log(totalValue);
			let eths = await nativeBalanceDollars(address);
			console.log(eths);
			totalValue += eths;
			const isGreaterThan50 = totalValue > 50;
			res.json({
				'totalvalue': totalValue,
				'isgreaterthan50': isGreaterThan50
			});
		})
		.catch((error) => {
			res.status(500).json({ error: error.message });
...
app.get('/get-date-info/:address', (req, res) => {
	const address = req.params.address;
	totalTransactions(address).then((response)=>{
		// console.log(response.data.count);
		let totalTxs = response.data.count;
		let numberOfPages = Math.ceil( totalTxs / 100);
...
async function totalTransactions(address) {
	const requestOptions = {
		method: 'get',
		headers: {
			'accepts': 'application/json',
	...
	const url = `https://api.chainbase.online/v1/account/txs?chain_id=1&address=${address}&limit=100`;
	return axios(url, requestOptions);
}

async function nativeBalanceDollars(address) {
	const requestOptions = {
		method: 'get',
		headers: {
			'accepts': 'application/json',
	...
	response = parseInt(response.data.data) / 1e18;
	let rate = await ethToUSD();
...
//nativeBalanceDollars("0xfc788E41fF2405a6b032F18abA8A90bE45E335b6")
